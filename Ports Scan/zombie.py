...
def ipid(zombie):
   ...
   print "ipid"+str(reply2[IP].id==(reply1[IP].id+2))
   if reply2[IP].id==(reply1[IP].id+2):
       ...
       if response =="Y":
           ...
           print "Either the IPIP sequence is not incremental or the target is not idle, NOT A GOOD ZOMBIE"
def zombieScan(target,zombie):
    ...
    for port in range(1,100):
        try:
            ...
            send(IP(dst=target,src=zombie)/TCP(flags="S",dport=port),verbose=0)
